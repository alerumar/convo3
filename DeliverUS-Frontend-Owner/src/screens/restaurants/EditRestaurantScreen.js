...
export default function EditRestaurantScreen ({ navigation, route }) {
  ...
  const [percentageProduct, setPercentageProduct] = useState(false)
  ...
  const updateRestaurant = async (values) => {
    setBackendErrors([])
    if (values.percentage !== 0 && !percentageProduct) {
      setPercentageProduct(true)
    } else {
      setPercentageProduct(false)
      try {
    ...
    <Formik
      ...
      {({ handleSubmit, setFieldValue, values }) => (
        <ScrollView>
            ...
            <View style={{ width: '60%' }}>
              ...
              <View style ={ styles.percentageContainer}>
                <Pressable onPress={() => {
                  ...
                  <MaterialCommunityIcons name={'arrow-up-circle'}
                ...
                <Pressable onPress={() => {
                  ...
                  <MaterialCommunityIcons name={'arrow-down-circle'}
                    color={GlobalStyles.brandSecondaryTap}
                    size={40}/>
                </Pressable>
          ...
          <ConfirmationModal
            isVisible={percentageProduct}
            onCancel={() => setPercentageProduct(false)}
