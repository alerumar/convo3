...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const [restaurantsToBePromoted, setRestaurantsToBePromoted] = useState(null)
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        <TextSemiBold>Shipping: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>
          {item.shippingCosts.toFixed(2)}€</TextSemiBold></TextSemiBold>
          { item.promoted &&
          <TextRegular textStyle={[styles.promotedMessage, { color: GlobalStyles.brandPrimary, borderColor: GlobalStyles.brandSuccess }]}>
              ¡En promocion!
          </TextRegular>
          }
        </View>
        ...
        <Pressable
            onPress={() => { setRestaurantsToBePromoted(item) }}
            style={({ pressed }) => [
              {
                backgroundColor: pressed
  ...
  const promoteRestaurant = async (restaurant) => {
    try {
      ...
      await fetchRestaurants()
      setRestaurantsToBePromoted(null)
      showMessage({
  ...
  return (
    ...
    <ConfirmationModal
      isVisible={restaurantsToBePromoted !== null}
      onCancel={() => setRestaurantsToBePromoted(null)}
      onConfirm={() => promoteRestaurant(restaurantsToBePromoted)}>
